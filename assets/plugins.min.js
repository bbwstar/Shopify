...
!function(){$.fn.apusCountDown=function(a){return this.each(function(){new $.apusCountDown(this,a)})},$.apusCountDown=function(a,b){if(this.options=$.extend({autoStart:!0,LeadingZero:!0,DisplayFormat:"<div>%%D%% Days</div><div>%%H%% Hours</div><div>%%M%% Minutes</div><div>%%S%% Seconds</div>",FinishMessage:"Expired",CountActive:!0,TargetDate:null},b||{}),null!=this.options.TargetDate&&""!=this.options.TargetDate){this.timer=null,this.element=a,this.CountStepper=-1,this.CountStepper=Math.ceil(this.CountStepper),this.SetTimeOutPeriod=1e3*(Math.abs(this.CountStepper)-1)+990;var c=new Date(this.options.TargetDate),d=new Date;ddiff=new Date(this.CountStepper>0?d-c:c-d),gsecs=Math.floor(ddiff.valueOf()/1e3),this.CountBack(gsecs,this)}},$.apusCountDown.fn=$.apusCountDown.prototype,$.apusCountDown.fn.extend=$.apusCountDown.extend=$.extend,$.apusCountDown.fn.extend({calculateDate:function(a,b,c){var d=(Math.floor(a/b)%c).toString();return this.options.LeadingZero&&d.length<2&&(d="0"+d),"<b>"+d+"</b>"},CountBack:function(a,b){return a<0?void(b.element.innerHTML='<div class="apus-labelexpired"> '+b.options.FinishMessage+"</div>"):(clearInterval(b.timer),DisplayStr=b.options.DisplayFormat.replace(/%%D%%/g,b.calculateDate(a,86400,1e5)),DisplayStr=DisplayStr.replace(/%%H%%/g,b.calculateDate(a,3600,24)),DisplayStr=DisplayStr.replace(/%%M%%/g,b.calculateDate(a,60,60)),DisplayStr=DisplayStr.replace(/%%S%%/g,b.calculateDate(a,1,60)),b.element.innerHTML=DisplayStr,void(b.options.CountActive&&(b.timer=null,b.timer=setTimeout(function(){b.CountBack(a+b.CountStepper,b)},b.SetTimeOutPeriod))))}})}(jQuery);
...
$(document).ready(function(){
    ...
    $('.testimonials .grid').each(function(){  
      ...
      $('.grid__item',this).height(highestBox);
